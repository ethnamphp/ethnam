<title>
遷移時のデフォルトマクロを指定する。 - Ethna - PHPウェブアプリケーションフレームワーク</title>
...
<!-- ??BEGIN id:body --> [Ethna](index.html) > [ドキュメント](ethna-document.html) > [開発マニュアル](ethna-document-dev_guide.html) > [遷移先定義、テンプレートの扱い方](ethna-document-dev_guide-forward.html) > 遷移時のデフォルトマクロを指定する。 
