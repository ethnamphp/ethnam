...
こうして取得した Ethna_Loggerクラスのインスタンスに対して、ログレベルとメッセージを引数として以下の形で直接指示します。エラー関連の処理については [エラー処理](error.md) のページも参照してください。
...
通知するレベルを変更するには、level の値を変更します。 [ログレベルの説明](log.md#md5faa05)も参照して下さい。
